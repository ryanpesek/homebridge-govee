...
export default class {
  constructor(platform, accessory) {
    ...
    // Add the auto switch service if it doesn't already exist
    this.autoSwitchService = this.accessory.getService(this.hapServ.Switch)
    ...
    if (!this.leakService) {
      this.leakService = this.accessory.addService(this.hapServ.LeakSensor)

      // Adding the characteristic here avoids Homebridge characteristic warnings
      //this.leakService.addCharacteristic(this.eveChar.LastActivation)
    }
    this.cacheLeak = this.leakService.getCharacteristic(this.hapChar.LeakDetected).value ? 'on' : 'off'
    ...
    // Add the set handler to the auto switch on/off characteristic
    this.autoSwitchService.getCharacteristic(this.hapChar.On).onSet(async (value) => {
      await this.internalAutoStateUpdate(value)
    })
    this.cacheAutoSwitchState = this.autoSwitchService.getCharacteristic(this.hapChar.On).value ? 'on' : 'off'
  ...
  async internalAutoStateUpdate(value) { 
    // nothing to do
  }
  ...
  externalUpdate(params) {
    ...
    (params.commands || []).forEach((command) => {
      ...
      // 'qhcAAAEAAAAAAAAAAAAAAAAAALw=' means humidifier not running
      if (command === "qhcAAAEAAAAAAAAAAAAAAAAAALw=") {
        if (this.cacheAutoSwitchState != 'off') {
          this.cacheAutoSwitchState = 'off'
          this.autoSwitchService.updateCharacteristic(this.hapChar.On, false)
          this.accessory.log(`setting auto switch to off as humidifier stopped running`)
        }        
      }

      // 'qhcAAAAAAAAAAAAAAAAAAAAAAL0=' means humidifier not running
      if (command === "qhcAAAAAAAAAAAAAAAAAAAAAAL0=") {
        if (this.cacheAutoSwitchState != 'on') {
          this.cacheAutoSwitchState = 'on'
          this.autoSwitchService.updateCharacteristic(this.hapChar.On, true)
          this.accessory.log(`setting auto switch to on as humidifier started running`)
        }        
      }

      if (command.startsWith("qhcAAQ")) {
        if (this.cacheLeak != 'on') {
          this.cacheLeak = 'on'
          this.leakService.updateCharacteristic(this.hapChar.LeakDetected, true)
          this.accessory.log(`setting leak service to true as low water detected`)
        }
      } else if (command.startsWith("qhcAAA")) {
        if (this.cacheLeak != 'off') {
          this.cacheLeak = 'off'
          this.leakService.updateCharacteristic(this.hapChar.LeakDetected, false)
          this.accessory.log(`setting leak service to false as no lower water detected`)
        }
      }
